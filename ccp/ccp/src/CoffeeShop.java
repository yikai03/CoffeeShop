...
public class CoffeeShop {
    ...
    public void AddCustomerToOrderQueue(Customer customer) throws InterruptedException{
        ...
        synchronized(QueueOfOrder){//Synchronize the queue of order
            ...
            System.out.println(customer.GetMyColor()+ "Customer " + customer.GetCustomerId() + " Arrived in Coffee Shop." + 
            " ShareSeat: " + customer.isShareSeat() + customer.GetMyColorReset());
        ...
        if (success) {//If the customer is successfully added to the queue
            ...
            ": Order Queue is too long, I am leaving. Bye" + customer.GetMyColorReset());       
    ...
    public void AddCustomerToSeat(){ 
        ...
        if(customer!=null){//If the customer is not null
            ...
            System.out.println(customer.GetMyColor() + "Customer " + customer.GetCustomerId()+ 
            ": I have taken my " + customer.getOrder() + customer.GetMyColorReset());
            System.out.println(customer.GetMyColor() + "Customer " + customer.GetCustomerId()+ 
            ": I am now trying to find a seat" + customer.GetMyColorReset());
            while(customer.isInSeat() == false){//While the customer is not in seat
                ...
                //If customer who did not want to share table, cannnot find a seat after search all 
                //the table. He will be willing to share the table
                if(customer.isInSeat() == false && customer.isShareSeat() == false){
                    ...
                    System.out.println(customer.GetMyColor() + "Customer " + customer.GetCustomerId()
                     + ": I am now willing to share the table" + customer.GetMyColorReset());
                }
                //Since if customer thread will try lock a table for 1 second, after 5 table, it will 
                //be 5 second, add on the 1 second sleep, 6 will be total. 
    ...
    public void Work(Barista barista){        
        //While the queue of order is full and not closing time
        while(QueueOfOrder.remainingCapacity() == 5 && !barista.isClosingTime()){
            System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
            ": I am sleeping" + barista.getMyColorReset());//Print out that the barista is sleeping
            WaitCustomerToCome();//Wait for the customer to come
        }
        
        if(barista.isClosingTime() /*& QueueOfOrder.isEmpty()*/){//If it is closing time
            System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
            ": I am going home" + barista.getMyColorReset());//Print out that the barista is going home
            return;
        ...
        synchronized(QueueOfBaristaTakingOrder){
            try {
                ...
                if(customer!= null){//If the customer is not null
                    ...
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": Hi, May I take your order? (To Customer " + customer.GetCustomerId() + ")" + barista.getMyColorReset());
                    Thread.sleep(500);//Stimulate order time 
                    //Print out that the barista is taking the order                
                    System.out.println(customer.GetMyColor()+ "Customer " + customer.GetCustomerId()+ 
                    ": Hi, I want to order " + customer.getOrder() + ", and I pay for RM " + Payment(customer.getOrder()) 
                    + customer.GetMyColorReset());
    ...
    public int Payment(String order){
        if("Expresso".equals(order)){//If the order is expresso
            return ExPrice;//Return the price of expresso
        }else if("Cappuccino".equals(order)){//If the order is cappuccino
            return CappuPrice;//Return the price of cappuccino
        }else if("Juice".equals(order)){//If the order is juice
            return JuicePrice;//Return the price of juice
        }
        return 0;//Return 0 if the order is not expresso, cappuccino, or juice
    }
    
    public void MakeDrinkProcess(Barista barista, String machine){
        int processTime = 25;//Reset process time
        while(processTime <= 100){//While process time is less than 100
            //Print out the making drink process
            System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
            ": I am using " + machine + " for making " + barista.getCurrentWorkingDrink() + 
            " for Customer " + barista.getCurrentWorkingCustomerID() + " (" + processTime + "%)" + barista.getMyColorReset());
    ...
    public void MakeDrink(Customer customer) throws InterruptedException{
        ...
        if(barista!=null){//If the barista is not null
            if("Expresso".equals(barista.getCurrentWorkingDrink())){//If the current working drink is expresso
                ...
                System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                ": I'm trying to use expresso machine (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Expresso)"+ barista.getMyColorReset());
                //Lock the expresso machine
                ExpressoMachine.lock();
                    //Print out that the barista is using expresso machine
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": I am using expresso machine (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Expresso)" + barista.getMyColorReset());
                    //Make the drink process
                    MakeDrinkProcess(barista, "Expresso Machine");
                    //Print out the machine is not in use
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": I am finished using the expresso machine (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Expresso)" + barista.getMyColorReset());
                    //Print out that the expresso is ready  
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": Expresso for Customer " + barista.getCurrentWorkingCustomerID() + " is ready" + barista.getMyColorReset());
                ...
                //Print out that the barista is trying to use expresso machine
                System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                ": I'm trying to use expresso machine (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Cappuccino)"+ barista.getMyColorReset());
                ExpressoMachine.lock();
                    //Print out that the barista is using expresso machine
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": I am using expresso machine (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Cappuccino)" + barista.getMyColorReset());
                    MakeDrinkProcess(barista, "Expresso Machine");//Make the drink process
                    //Print out the machine is not in use
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": I am finished using the expresso machine (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Cappuccino)" + barista.getMyColorReset());
                ExpressoMachine.unlock();                
                //Print out that the barista is trying to use milk frother
                System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                ": I'm trying to use milk frother (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Cappuccino)"+ barista.getMyColorReset());
                MilkMachine.lock();
                    //Print out that the barista is using milk frother
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": I am using milk frother (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Cappuccino)" + barista.getMyColorReset());
                    MakeDrinkProcess(barista, "Milk Frother");//Make the drink process
                    //Print out the machine is not in use
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": I am finished using the milk frothing machine (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Cappuccino)" + barista.getMyColorReset());
                    //Print out that the cappuccino is ready
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": Cappuccino for Customer " + barista.getCurrentWorkingCustomerID() + " is ready" + barista.getMyColorReset());
                    barista.setCustomerTakenDrink(true);//Set the customer has taken the drink
                    synchronized(QueueOfFindingSeat){//Synchronize the queue of finding seat
                        QueueOfFindingSeat.notifyAll();//Notify all the customer to find seat
                    }
                ...
                System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                ": I'm trying to use juice tap (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Juice)" + barista.getMyColorReset());
                JuiceTap.lock();
                    //Print out that the barista is using juice tap
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": I am using juice tap (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Juice)" + barista.getMyColorReset());
                    MakeDrinkProcess(barista, "Juice Tap");//Make the drink process 
                    //Print out the machine is not in use
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": I am finished using the Juice Tap (For Customer " + barista.getCurrentWorkingCustomerID() + "'s Juice)" + barista.getMyColorReset());  
                    //Print out that the juice is ready                                      
                    System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
                    ": Juice for Customer " + barista.getCurrentWorkingCustomerID() + " is ready" + barista.getMyColorReset());
                    barista.setCustomerTakenDrink(true);//Set the customer has taken the drink
                    synchronized(QueueOfFindingSeat){//Synchronize the queue of finding seat
                        QueueOfFindingSeat.notifyAll();//Notify all the customer to find seat
                    }
                ...
                JuiceTap.unlock();
            }else{System.out.println(barista.getMyColor() + "\t\t\t\t\tBarista " + barista.getBaristaId() + 
            " is confused" + barista.getMyColorReset());}//else, the barista is confused
        }
    }
    ...
    public void showTotalDrink(){//Show the total drink
        ...
        System.out.println("Sales of Expresso " + "(" +TotalEx+ "* RM" + ExPrice + "): RM" +TotalEx*ExPrice);
        System.out.println("Sales of Cappuccino " + "(" +TotalCappu+ "* RM" + CappuPrice + "): RM" +TotalCappu*CappuPrice);
        System.out.println("Sales of Juice " + "(" +TotalJuice+ "* RM" + JuicePrice + "): RM" +TotalJuice*JuicePrice);
